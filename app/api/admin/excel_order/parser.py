# app/api/admin/excel_order/parser.py
...
def has_invalid_font_color(cell) -> bool:
    """
    フォントカラーが異常な状態かどうかをチェック
    font.color.rgb が "Values must be of type" エラーを返すケースを検出
    """
    try:
        # font.color.rgb を文字列化して "Values must be of type" が含まれるかチェック
        if cell.font and cell.font.color:
            color_str = str(cell.font.color.rgb)
            if "Values must be of type" in color_str:
                return True
    except:
        pass
...
async def parse_excel_order(file_content: bytes, filename: str) -> dict:
    ...
    for r in range(START_ROW, END_ROW + 1):
        ...
        # 罫線があり、かつフォントカラーが正常（"Values must be of type" エラーがない）
